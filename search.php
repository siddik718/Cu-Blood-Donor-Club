<?php
	$num = 0;
	if(!empty($_POST['srch']) && isset($_POST['srch'])) {
		include 'config.php';
		$val = $_POST['srch'];

		if($val == "AB+" || $val == "AB-" || $val == "B+" || $val == "B-" || $val == "A+" || $val == "A-" || $val == "O+" || $val == "O-") {
			$sql = "Select *from `user` where `bgroup` = '$val'";
			$result = mysqli_query($con,$sql);
			$num = mysqli_num_rows($result);
		}else {
			$sql = "Select *from `user` where `address` = '$val'";
			$result = mysqli_query($con,$sql);
			$num = mysqli_num_rows($result);
		}
	}
...
<html class="no-js" lang="en">
   <head>
      <meta charset="utf-8">
      <title>CU BDC Search Page</title>
   ...
   <body>
      ...
      <div class="right_side">
         <div class="grp_container">
            <h5 class="grp_title">Donor List</h5>
